...
M.update = function(cur)
  ...
  if cur == 'cur' and 1 then
    if vim.tbl_contains(M.markdowns_fts, ext) == true then
      cur = fname
      M.notify('[updating] markdown document cur: `' .. vim.fn.fnamemodify(fname, ':t') .. '`')
    else
      M.notify('update markdown document cur [failed]: not markdown: `' .. vim.fn.fnamemodify(fname, ':t') .. '`')
      return
    end
  else
