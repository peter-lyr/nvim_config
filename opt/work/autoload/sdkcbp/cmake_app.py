...
with open(os.path.join(rootdir, "CMakeLists.txt"), "wb") as ff:
    ...
    for i, j, k in os.walk(rootdir):
        for f in k:
            if f.split(".")[-1] == "cbp":
                if f == "app.cbp":
                    if projectName != "projects" and projectName not in i.replace(
                        rootdir, ""
                    ):
                        continue
                ...
                files = [
                    os.path.normpath(os.path.join(i, file))
                    .replace("\\", "/")
                    .replace(rootdir, "")
                    .strip("/")
                    for file in files
        ...
        if key.split(".")[0].split("\\")[-1] == "app":
            ...
            ff.write(
                (
                    ...
                    % (libname, " ${PROJECT_SOURCE_DIR}/".join(files))
                ).encode("utf-8")
            )
            bb = [
...
projFileNames = [
    fname.replace("/", "\\") for fname in projFileNames + projFileNames_last
]
