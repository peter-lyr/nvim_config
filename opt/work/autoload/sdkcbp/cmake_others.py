    ...
    for i, j, k in os.walk(rootdir):
        for f in k:
            if f.split(".")[-1] == "cbp":
                ...
                directories = [
                    os.path.normpath(os.path.join(i, directory))
                    for directory in directories
                ]

                ff.write(
                    b"file(GLOB_RECURSE SOURCE_FILES ${CMAKE_CURRENT_SOURCE_DIR}/*.c)\n"
                )
                ff.write(
                    b"file(GLOB_RECURSE ASM_SOURCE_FILES ${CMAKE_CURRENT_SOURCE_DIR}/*.S)\n"
                )
                ff.write(
                    b"add_executable(${PROJECT_NAME} ${SOURCE_FILES} ${ASM_SOURCE_FILES})\n"
                )
                bb = [
                    "target_include_directories(${PROJECT_NAME} PUBLIC ${PROJECT_SOURCE_DIR}/%s)"
                    % aa.replace(rootdir, "").strip("\\").strip("/").replace("\\", "/")
                    for aa in directories
                ]
