    ...
    if vim.api.nvim_buf_get_option(ev.buf, 'filetype') == 'minimap' then
      vim.fn.timer_start(20, function()
        if vim.fn.getcurpos()[5] > 3 and (vim.fn.trim(vim.fn.execute 'norm g8') == '20') == true then
          ...
          vim.keymap.set({ 'n', }, '`', function() esc(1) end, { buffer = ev.buf, desc = 'MiniMap esc', })
          vim.keymap.set({ 'v', }, '`', function() esc(2) end, { buffer = ev.buf, desc = 'MiniMap esc', })
          ...
          vim.keymap.set({ 'n', }, '<tab>', function() cr(1) end, { buffer = ev.buf, desc = 'MiniMap cr', })
          vim.keymap.set({ 'v', }, '<tab>', function() cr(2) end, { buffer = ev.buf, desc = 'MiniMap cr', })
          vim.keymap.set({ 'n', }, 'a', function() cr(1) end, { buffer = ev.buf, desc = 'MiniMap cr', })
          vim.keymap.set({ 'v', }, 'a', function() cr(2) end, { buffer = ev.buf, desc = 'MiniMap cr', })
